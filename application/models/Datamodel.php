...
class datamodel extends CI_Model { //mengextands CI_Model
  ...
  public function __construct(){
    $sql = $this->db->query("SELECT katadasar FROM tb_katadasar")->result_array();
		$ar = array();
		foreach($sql as $row){
			$ar[] = $row['katadasar'];
		}

    $sqlnormalisasi = $this->db->query("SELECT slang,formal FROM normalisasi")->result_array();
    $normal = array();
    foreach($sqlnormalisasi as $row){
      $normal[] = $row;
		}

		$this->kamus = $ar;
    $this->normalisasi = $normal;
  }
  public function stem($string){
    /**
		 * $cleaningChar = menentukan karakter apa yang akan dihapus melalui variabel ini
		 */
    /**
			 * $stopWord berfungsi untuk menampung stopword dari masing kata
			 * $stemming berfungsi untuk menampung stemming atau kata dasar menggunakan nazief
    */
    $cleaningChar = CLEANING_CHAR;
    $link = LINK;

    $input = filter_var(strtolower($string), FILTER_SANITIZE_STRING);
		$result = preg_replace($link, "", $input);
    $result = preg_replace($cleaningChar, "", $result);
    ...
    $input = [];
		foreach($pecah as $item){
			if(strlen($item) > 0){
        // stemming
				$input[] = $this->cekKata($item);
        $this->input[] = $this->cekKata($item);
			}
		}
    return $input;
  }
  public function normalisasi($text){
    $sql = $this->normalisasi;
    $result = $text;
    foreach($sql as $row){
      
      $result = str_replace($row['slang'], $row['formal'], $text);
		}
    // echo "<pre>";
    // print_r($result);
    // exit();
  ...
  public function stopword($input){
		$sql = $this->db->query("SELECT stopword FROM stopword")->result_array();
		foreach($sql as $row){
    ...
    return $saved = array_filter($saved);
	}
  public function stopwordSingle($input){
		$sql = $this->db->query("SELECT stopword FROM stopword")->result_array();
		foreach($sql as $row){
			$stopword[] = $row['stopword'];
		}
